...
for i, state in enumerate(sfs.fSFs):
    ...
    text = (r"$E_{x} = $" + f"{un.nominal_value(ex):.2f}") if state != "g0" else "g.s."
    c2s = df.loc[df["name"] == state]["sf"].values[0]
    text2 = r"$C^2S = $" + f"{c2s:.1uS}"
    xpos = 0.65 if i != 1 else 0.45
    pos = (xpos, 0.725)
    ax.annotate(
        text + "\n" + text2,
        xy=pos,
...
fig.savefig("./Outputs/ang.png", dpi=300)
plt.show()
