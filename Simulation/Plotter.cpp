             ...
             const std::string& light, const std::string& heavy, double T1, int neutronPS, int protonPS)
{
    ...
    {
        // Get DF
        ROOT::RDataFrame df("SimulationTTree", E796Simu::GetFile(beam, target, light, Ex, neutronPS, protonPS));

        // Book histograms
        auto hEx {df.Histo1D(
            {TString::Format("hEx%d", idx),
             TString::Format("Ex = %.2f MeV with nPS = %d and pPS = %d;E_{ex} [MeV]", Ex, neutronPS, protonPS), nBinsEx,
             -10., 60.},
            "Eex", "weight")};
        auto hKin {df.Histo2D(
            {TString::Format("hKin%d", idx),
             TString::Format("Ex = %.2f MeV with nPS = %d and pPS = %d;#theta_{Lab} [#circ];E_{Vertex} [MeV]", Ex,
                             neutronPS, protonPS),
