...
void DebugAng()
{
    ...
    for(int i = 0; i < nodes.size(); i++)
    {
        auto iv {std::make_shared<Angular::Intervals>(thetaCMMin, thetaCMMax, E796Fit::Exdt, thetaCMStep)};
        ivs.push_back(iv);
        // Fill them
    ...
    for(int i = 0; i < nodes.size(); i++)
    {
        ...
        fitter.Configure(("./Outputs/" + labels[i] + ".root"), {});
        fitter.Run();
    ...
    for(int i = 0; i < nodes.size(); i++)
    {
        ...
        for(const auto& peak : peaks)
            effs.back().Add(peak, TString::Format("/media/Data/E796v2/Simulation/Outputs/Eff_study/%s_%s.root",
                                                  labels[i].c_str(), peak.c_str())
                                      .Data());

        effs.back().Draw(true, "Eff for " + labels[i]);
        // Experiments
        // WARNING: Nt must change with eff length, so correct considering LISE calculation
        exps.push_back({4.5625e20, 279932, 30000});
        // Run!
