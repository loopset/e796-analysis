...
void nops_Ang(bool isLab = false)
{
    ...
    int nps {static_cast<int>(2 + 0)}; // 2 nps +  0 contamination
    ...
    // WARNING: set ps0 amp to 0
    std::vector<double> psamps {};
    for(int i = 0; i < ivs.GetSize(); i++)
        psamps.push_back(0);
    fitter.SetFixAmpPS(0, psamps);
    fitter.SetFixAmpPS(1, psamps);
    ...
    inter.FitComp();
    inter.WriteComp("./Outputs/nops_sfs.root");
