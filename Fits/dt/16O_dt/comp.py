...
for file in files:
    ...
    # Data is in MICRO BARN -> scale to MILLI BARN
    data[:, 1] *= 1e-3
...
def find_q_paper(
    dic: Dict[float, Any], ex: float, tol: float = 0.02
...
with open("./Outputs/reanalysis.pkl", "wb") as f:
    pickle.dump(reana, f)
with open("./Outputs/paper.pkl", "wb") as f:
...
for i, dictio in enumerate(sts):
    ax.errorbar(
        ...
        label="Purser" if i == 0 else "Reanalysis",
    )
ax.legend()
