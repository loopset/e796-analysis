...
## EM dataset
labels = [r"$2_{0}^{+}$", r"$2_{1}^{+}$", r"$3_{0}^{+}$"]
ls = [2, 2, 3]
em = [un.ufloat(5.9, 0.2), un.ufloat(1.3, 0.2), np.nan]
em = [BEL_to_beta(be, z, a, l, False) for be, l in zip(em, ls)]

## (d,d)
dd = [un.ufloat(0.3203, 0.0070), un.ufloat(0.1824, 0.0049), un.ufloat(0.2391, 0.0067)]
...
axs[0].errorbar(labels, unp.nominal_values(em), yerr=unp.std_devs(em), fmt="o", label="EM I.Zanon et al")
axs[0].errorbar(labels, unp.nominal_values(dd), yerr=unp.std_devs(dd), fmt="o", label=r"$^{20}$O(d,d) E796")
axs[0].errorbar(labels, unp.nominal_values(ppkhan), yerr=unp.std_devs(ppkhan), fmt="o", label=r"$^{20}$O(p,p) E.Khan")
axs[0].set_ylabel(r"$\beta_{L}$")
axs[0].legend(fontsize=14, frameon=True, fancybox=True, shadow=True)
axs[1].errorbar(labels, unp.nominal_values(mnmp), yerr=unp.std_devs(mnmp), fmt="o", label="E796")
axs[1].errorbar(labels, unp.nominal_values(mnmpkhan), yerr=unp.std_devs(mnmpkhan), fmt="o", label="E.Khan")
